...
- Improved the [`confidence`]({{site.structs}}ExtendedResult.html?src=c#confidence) algorithm for 1D barcode results. Users can get even more accurate results by configuring the `confidence` filter.
...
- Added the [`organizationID`]({{site.structs}}DMLTSConnectionParameters.html?src=c#organizationid) property for license authentication.
- Added a new attribute [`isMirrored`]({{site.structs}}TextResult.html?src=c#ismirrored) to the `TextResult` class. `isMirrored` returns whether the barcode is mirrored.
- Added a new attribute [`isDPM`]({{site.structs}}TextResult.html?src=c#isdpm) to the `TextResult` class. `isDPM` returns whether the barcode is recognized by the DPM mode.
...
- Fixed an issue that happens when calling initLicenseFromLTS if [`handShakeCode`]({{site.structs}}DMLTSConnectionParameters.html?src=c#handshakecode) is not set.
