...
- Added a new method [`DBR_GetInstancePoolStatus`]({{site.c_methods}}license.html#dbr_getinstancepoolstatus) and struct [`InstancePoolStatus`]({{ site.structs }}InstancePoolStatus.html?src=c) to provide a real-time view of how the concurrent instance licenses are being utilized. This applies only if you are using a concurrent instance license.
