...
- Added a optional parameter `unique_barcode_callback_func` to method [`start_video_mode`]({{site.python_methods}}video.html#start_video_mode) to set callback function to process unique barcode results which is triggered when the library finishes decoding a frame and finds unique barcodes.
